...
class SCAE(models.Model):
    ...
    def __init__(self, org_shape):
        ...
        c1 = SCAE.conv(original, 16, mp_flag=False)
        c2 = SCAE.conv(c1, 32)

        encoded = SCAE.conv(c2, 64)

        x = SCAE.deconv_scae(encoded, 32)
        y = SCAE.deconv_scae(x, 16)
...
def show_images(data, scae):
    x_test_in = data.x_test_in
    x_test_out = data.x_test_out
    decoded_imgs = scae.predict(x_test_in)
...
fig = True
data = DATA()
scae = SCAE(data.input_shape)
scae.summary()

history = scae.fit(data.x_train_in, data.x_train_out, epochs=epochs, batch_size=batch_size,
                   shuffle=True, validation_data=(data.x_test_in, data.x_test_out))

plot_loss(history)
plt.savefig('SCAE_LOSS.png')
plt.clf()
show_images(data, scae)
plt.savefig('SCAE_PRED3.png')
plt.show()
